  ...
  try {
    ...
    if (token) {
      await db.syncLog.update({
        ...
        data: { status: "completed", message: "Fanvue data sync initiated", finishedAt: new Date().toISOString() },
      });
    } else {
      await db.syncLog.update({
        where: { id: fanvueSync.id },
    ...
    await db.syncLog.update({
      ...
      data: { status: "completed", message: "Repo sync initiated", finishedAt: new Date().toISOString() },
    });
